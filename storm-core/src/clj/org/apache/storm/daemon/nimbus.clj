...
(defn update-heartbeat-cache [cache executor-id-beats all-executors timeout]
  (let [cache (select-keys cache all-executors)]
    (into {}
          (for [executor all-executors :let [curr (cache executor)]]
            [executor
             (update-executor-cache curr (get executor-id-beats executor) timeout)]
            ))))

(defn update-heartbeat-cache-hudie [cache executor-beats all-executor-ids timeout]
  (let [executor-id->executor (into {} (for [[^Executor executor hb] cache :let [executor-id [(:start-task-id executor) (:last-task-id executor)]]]
...
(defn- compute-topology->executors [nimbus storm-ids]
  ...
  (into {} (for [tid storm-ids]
             {tid (set (compute-executors nimbus tid))})))  ;;得到的是map key是topology-id value是[start-id,last-id]的集合， {topology-id, set([start-id,end-id], [start-id,end-id])}

(defn- compute-topology->executors-according-to-existing-assignments [existing-assignments]
  (into {} (for [[tid assignment] existing-assignments
                 :let [executor->node+port (:executor->node+port assignment)
                       executors (->> executor->node+port keys set)]]
             {tid executors})))

(defn- compute-topology->alive-executors [nimbus existing-assignments topologies topology->executors scratch-topology-id] ;; 这里得到的executor也是[start-id,last-id]的形式
...
(defn- compute-supervisor->dead-ports [nimbus existing-assignments topology->executors topology->alive-executors]
  (let [dead-slots (into [] (for [[tid assignment] existing-assignments
                                  :let [all-executors (topology->executors tid) ;; 这里的executor是[start-id,last-id]的形式
                                        alive-executors (topology->alive-executors tid) ;; 这里的executor是[start-id,last-id]的形式
...
(defn compute-new-scheduler-assignments [nimbus existing-assignments topologies scratch-topology-id]
  (let [conf (:conf nimbus)
        ...
        ;;得到的executor是[start-id,last-id]的形式
                   ...
                   (fn [^String storm-id operation]
                     (let [storm-cluster-state (:storm-cluster-state nimbus)
                           ...
                           beats (map-val :heartbeat (get @(:heartbeats-cache nimbus)
                                                          storm-id)) ;;beats是一个map {<executor-id, executor-heartbeat>}
